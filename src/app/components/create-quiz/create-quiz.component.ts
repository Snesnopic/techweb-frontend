  ...
  private prepareQuizData(): any {
    ...
    const questions = quizFormValue.questions.map((question: any) => {
      const correctAnswer =
        ...
        question.answers.find((a: any) => a.isCorrect)?.answerText ||
        "";
      return {
  ...
  questionsValidator(): ValidatorFn {
    return (control: AbstractControl): ValidationErrors | null => {
      ...
      for (const question of questions.controls) {
        ...
        // Validate answers
        const nonEmptyAnswers = answers.controls.filter(
          (a) => a.get("answerText")?.value.trim() !== "",
        );
        const hasCorrectAnswer = nonEmptyAnswers.some(
          (a) => a.get("isCorrect")?.value,
        );
